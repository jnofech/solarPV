function varargout = plot_GMap(varargin)
% function h = plot_google_map(varargin)
% Plots a google map on the current axes using the Google Static Maps API.
% This is a modified version of Zohar Bar-Yehuda's code on the MATLAB File
% Exchange (`plot_google_map.m`).
%
% USAGE:
    ...
    for idx = 1:2:length(varargin)
        switch lower(varargin{idx})
            ...
            case 'apikey'
                ...
                save(keyFile,'apiKey','apiKey_valid')
            case 'style'
            ...
            case 'zoom'
                zoom_override = varargin{idx+1};
            case 'lat'
                lat_override = varargin{idx+1};
            case 'latitude'
                lat_override = varargin{idx+1};
            case 'lon'
                lon_override = varargin{idx+1};
            case 'longitude'
                lon_override = varargin{idx+1};
            otherwise
...
if ~isnan(zoom_override)
    zoomlevel = round(zoom_override);
end
...
% Override lat/lon with EXACT user-specified values, if any
if ~isnan(lat_override)
    lat = lat_override;
end
if ~isnan(lon_override)
    lon = lon_override;
end
...
try
    ...
    apiKey_valid = true;
    % save key validity to file
    funcFile = which('plot_google_map.m');
    pth = fileparts(funcFile);
    keyFile = fullfile(pth,'api_key.mat');
    save(keyFile,'apiKey','apiKey_valid')
catch % error downloading map
    image(1, 'Parent', axHandle)
...
%     h = image(lonVect,latVect,uniImag, 'Parent', axHandle);
    h = image(flipud(uniImag), 'Parent', axHandle); set(axHandle,'visible','off'); set(axHandle,'xtick',[])
